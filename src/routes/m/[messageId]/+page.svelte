<script lang="ts">
	...
	const replyText = $derived(`@${data.creator.handle}\n\n${pageUrl}`);
...
<div class="max-w-lg mx-auto px-4 py-10">
	...
	<div class="flex items-center gap-3 mb-6">
		...
		<div>
			...
			<p class="text-xs text-slate-500 dark:text-slate-400">@{data.creator.handle}</p>
		</div>
	</div>

	<!-- メッセージ本文 -->
	<div class="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm mb-4">
		<p class="text-slate-800 dark:text-slate-200 leading-relaxed whitespace-pre-wrap">{data.message.body}</p>

		{#if data.message.imageUrls.length > 0}
			<div class="mt-4 flex flex-wrap gap-2">
				{#each data.message.imageUrls as url}
					<a href={url} target="_blank" rel="noopener noreferrer">
						<img src={url} alt="" class="rounded-lg max-h-48 max-w-full object-cover border border-slate-200 dark:border-slate-700" />
					</a>
				{/each}
			</div>
		{/if}

		<p class="mt-4 text-xs text-slate-400 dark:text-slate-600">
	...
	<a
		...
		class="flex items-center justify-center gap-2 w-full bg-sky-500 hover:bg-sky-600 text-white font-medium py-3 rounded-xl transition-colors"
	>
